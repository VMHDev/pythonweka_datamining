...
def main(argv):
    lstNameItem = []
    dataAP = ReadFileCSV('input.csv', lstNameItem)
    if(len(dataAP) == 0):
        return
    minsupp = 0.3
    minconf = 0.8
    ...
    dataAss = GenAssociationRules(datars, lstTrans, minconf)
    print('\tTìm được: ' + str(len(dataAss)) + ' luật kết hợp')      
    for item in dataAss:
        print(item.ToString())

    dataAss.clear()
    dataAss = GenAssociationRules2(datars, lstTrans, minconf)
    print('\tTìm được thêm: ' + str(len(dataAss)) + ' luật kết hợp (có các luật trùng)!')      
    for item in dataAss:
        print(item.ToString())
